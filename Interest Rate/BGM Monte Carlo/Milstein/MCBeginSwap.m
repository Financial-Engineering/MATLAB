 ...
 % tmpcount = 1;
  ...
  while t<=T(alpha), %simulate the path for each forward rate, till beginning of swap
      ...
      for k=startk:beta-1,
	    ...
	    logF(k+1)=logF(k+1)+ sigma_Tk_t*drift_sum*dt - (sigma_Tk_t*sigma_Tk_t*dt)/2 + sigma_Tk_t*dZ(tmp_col)*sqrt(dt); % ,tmp_col);
        %logF(k)=logF(k)+ sigma_Tk_t*drift_sum*dt - (sigma_Tk_t*sigma_Tk_t*dt)/2 + sigma_Tk_t*dZ(tmp_col)*sqrt(dt); % before
      end
      %tmpcount=tmpcount+1;
      t=t+dt;
   end 
