function VM = solvingVolMatrix2(ti,F,rho,V)
% --------------------------------------------------------------
% VM = solvingVolMatrix2(ti,F,rho,V)
% This function recovers the entire swaption matrix.
%
...
for alpha = 1:n2/2
    for beta = alpha+1:n2/2+alpha
        if(alpha~=1 && beta==alpha+n2/2)
            sig(beta,alpha+1) = solvingMyFunc2(alpha,beta,ti,F,rho,sig,V);
            for k = 2:alpha
                sig(beta,k) = sig(beta,alpha+1);
            end
        else
