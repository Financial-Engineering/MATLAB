function vol = solvingMyFunc1(alpha,beta,ti,F,rho,sig,V)
% --------------------------------------------------------------
% vol = solvingMyFunc1(alpha,beta,ti,F,rho,sig,V)
...
if(alpha>n2/2+1 || beta>n2)
    fprintf('Swap tenor is outside the tenor dates.\n');
    return;
end
if(alpha~=1 && beta==n2/2+alpha)
    fprintf('solvingMyFunc1 is not the correct function to use in this case.\n');
    return;
end
if (n2~=n3)
...
for i = alpha+1:beta-1
    for j = alpha+1:beta-1
        ...
        summing1 = summing1 + wComputation(i,alpha,beta,F,ti)*wComputation(j,alpha,beta,F,ti)*F(i)*F(j)*rho(i,j)*innersumming1;
    end
end
summing2 = 0;
for j = alpha+1:beta-1
    innersumming2 = 0;
    for h = 1:alpha-1
        innersumming2 = innersumming2 + tau(h)*sig(beta,h+1)*sig(j,h+1);
    end
...
innersumming4 = 0;
for h = 1:alpha-1
    innersumming4 = innersumming4 + tau(h)*sig(beta,h+1)^2;
end
...
A = summing5;
B = summing3;
C = summing1 + summing2 + summing4 - summing6;
