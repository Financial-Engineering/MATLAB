function P = AsianQuantoBasket(CP,S,K,wa,wf,Spg,X,df,r,b,sigS,sigX,rhoij,rhoi,ti,t,T)
% -------------------------------------------------------------------------
...
if (nf~=rsize(1,1) || nf~=bsize(1,2) || nf~=length(ti))
    fprintf('Array of inputs do not match.\n');
    return;
end
...
for i= 1:na
    for j = m+1:nf
        bstar(i,j) = b(i,j) - rhoi(i)*sigS(i,j)*sigX(i,j);
